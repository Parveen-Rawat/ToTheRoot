...
def _load():
    ...
    if _collection is None:
        ...
        _collection = client.get_collection("cpts_notes")
        _embed_model = SentenceTransformer("all-MiniLM-L6-v2")
        print(f"[+] Loaded {_collection.count()} chunks from your CPTS notes")

def get_rag_suggestions(findings: list, n_results: int = 4) -> dict:
    _load()
    suggestions = {}
    ...
    for finding in findings:
        ...
        query_info = None
        for key in SERVICE_QUERY_MAP:
            if key in service:
                query_info = SERVICE_QUERY_MAP[key]
                break
